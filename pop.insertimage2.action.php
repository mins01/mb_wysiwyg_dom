...
//--------------------------------------------------------------------------
if(strlen($_FILES['inputFile']['name'])>1){
	$uploaded = true;
//print_r($_FILES);
	$filename_arr = pathinfo($_FILES['inputFile']['name']);
	/*
...
echo $path_parts['filename'], "\n"; // since PHP 5.2.0
	*/
	...
	if(strpos($_FILES['inputFile']['type'],'image')!==false){
		...
		if(function_exists("md5_file")){ //4.2.0 이상에서만 존재
			$md5_file = md5_file ($_FILES['inputFile']['tmp_name']);
		}else{
			$temp_file = file($_FILES['inputFile']['tmp_name']);
		...
		if($_FILES['inputFile']['size']>$max_size){
			$error_msg = "file size too big! \\n[{$_FILES['inputFile']['size']}>$max_size] byte";
			$result = false;
			$uploaded = false;
		}else{
			...
			if(is_file($uploaded_file)){
				$error_msg = 'Success : Upload Success (Exists File)';
				//같은 파일이 있으므로 업로드 하지 않고 그 파일을 사용한다
				//md5 를 사용하기 때문에 완벽하지는 않다. 하지만 거의 중복될 일은 없다.
			}else if (move_uploaded_file($_FILES['inputFile']['tmp_name'], $uploaded_file)) {
				@chmod($uploaded_file,0777);
				// print "파일이 존재하고, 성공적으로 업로드 되었습니다.";
				$error_msg = 'Success : Upload Success';
				$result = true;
	//			print "추가 디버깅 정보입니다:\n";
			} else {
